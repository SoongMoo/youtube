...
<c:forEach items="${list }" var="dto">	
	<tr><th width="15%">
			...
			<c:if test="${!empty dto.inquireAnswer }">답변 완료</c:if>
		</th>
		<th>${dto.memberNum }</th>
		<th>${dto.inquireDate }</th></tr>
	...
	<tr><td colspan="3">${fn:replace( dto.inquireContent, newLine, "<br />") }
				<c:if test="${!empty dto.inquireAnswer }">						
					<button type="button" style="float:right;" id="ingu_${dto.inquireNum}"
							onclick="answerOpen('${dto.inquireNum}');">답변열기</button>
				</c:if>
		</td></tr>
	<c:if test="${!empty dto.inquireAnswer }">
	<tr style="display:none;" id="ingu${dto.inquireNum}" ><td>답변</td><td colspan="2">
				${fn:replace( dto.inquireAnswer, newLine, "<br />") }
			</td></tr>
	</c:if>
</c:forEach>
</table>
